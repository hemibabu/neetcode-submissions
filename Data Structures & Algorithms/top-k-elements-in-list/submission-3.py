class Solution:
    def topKFrequent(self, nums: List[int], k: int) -> List[int]:
        ...
        for n,c in count.items(): # unpacking
            buc_sort[c].append(n)
    
        # emplty list creation to store result
        result = []
        # read the bucket in reverse and append k number of n's in the list
        for c in range(len(buc_sort) - 1, 0, -1): # loop through the count
            for n in buc_sort[c]: # and then acces the numbers assigned in the buckets
                result.append(n) # to append those numbers
                if len(result) == k:
                    return result